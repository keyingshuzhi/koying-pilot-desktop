...
it('skips hook setup when production dependencies omit lefthook', () => {
  const root = mkdtempSync(join(tmpdir(), 'dsh-lefthook-production-'))
  fixtures.push(root)
  mkdirSync(join(root, 'scripts'))
  copyFileSync(installer, join(root, 'scripts', 'install-lefthook.mjs'))

  const result = spawnSync(process.execPath, ['scripts/install-lefthook.mjs'], {
    cwd: root,
    encoding: 'utf8',
